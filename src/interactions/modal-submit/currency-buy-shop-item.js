...
import { colours, autoArray, strip } from "@magicalbunny31/pawesome-utility-stuffs";
...
export default async interaction => {
   ...
   userItems.push(
      ...autoArray(quantityWanted, () =>
         ({
            ...
            ref:          interaction.client.firestoreLegacy.collection(`currency`).doc(interaction.guild.id).collection(`users`).doc(itemSeller)
         })
      )
   );
   ...
   const taxToUserCurrencyDocRef  = interaction.client.firestoreLegacy.collection(`currency`).doc(interaction.guild.id).collection(`users`).doc(taxToUser);
   ...
   embeds[0]
      ...
      .setDescription(strip`
         ...
         >>> ${bunnyShopShopkeeper.purchase(interaction.client.allEmojis)}
      `)
      .setFooter({
         text: strip`
